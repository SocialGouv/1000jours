import * as HtmlUtils from "./html.util";
...
describe("Html utils", () => {
  describe("Fix video content", () => {
    it("should replace 'figure' and 'oembed' tags by 'iframe'", () => {
      ...
      expect(HtmlUtils.fixMediaContent(originalContent)).toBe(expectedContent);
    });
    ...
    it("should replace youtube links (https://youtube.com/watch?v=) by youtube embed links", () => {
      ...
      expect(HtmlUtils.fixYoutubeLinkContent(originalContent)).toBe(
        expectedContent
      );
    });
  ...
  describe("fix list content", () => {
    it("should replace 'ul' tag by 'ul' with 'max-width'", () => {
      ...
      expect(HtmlUtils.fixListContent(originalContent, screenWidth)).toBe(
        expectedContent
      );
    });
  });
